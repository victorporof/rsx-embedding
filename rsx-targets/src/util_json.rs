...
pub fn make_constellation<S, R>(setup: S, render: R) -> (DOMTree, ResourceGroup)
where
    S: Fn(&mut ResourceGroup),
    R: Fn() -> DOMTree
{
    let files = FileCache::new().unwrap();
    let images = ImageCache::new(TImageKeysAPI::new(())).unwrap();
    let fonts = FontCache::new(TFontKeysAPI::new(())).unwrap();
...
pub fn generate_display_list(tree: &mut DOMTree, width: u32, height: u32) -> DisplayList {
    tree.reflow_subtree(width, height, LayoutReflowDirection::LTR);
    DisplayList::from(tree)
}
...
pub fn take_resource_updates(resources: &mut ResourceGroup) -> ResourceUpdates {
    ...
    let mut updates = ResourceUpdates::with_capacity(images.len() + fonts.len());
